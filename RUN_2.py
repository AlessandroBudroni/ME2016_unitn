...
def test():
    ...
    scaler_1 = None
    with open('output/RUN_2_scaler_1.pickle', 'rb') as handle:
        scaler_1 = pickle.load(handle)

    detector_1 = None
    with open('output/RUN_2_classifier_1.pickle', 'rb') as handle:
    ...
    scaler_2 = None
    with open('output/RUN_2_scaler_2.pickle', 'rb') as handle:
        scaler_2 = pickle.load(handle)

    detector_2 = None
    with open('output/RUN_2_classifier_2.pickle', 'rb') as handle:
    ...
    for p in eff_tweets:
        ...
        if output[p] == 'fake':
            label = -1
        elif output[p] == 'unknown':
            label = 0

        print(p, '\t', posts_dict[p], '\t', label)

    with open('second_run.txt', 'w') as csvfile:
